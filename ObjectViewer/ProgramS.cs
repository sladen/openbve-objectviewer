				...
				for (int i = 0; i < Args.Length; i++) {
					if (!SkipArgs[i] && System.IO.File.Exists(Args[i])) {
						try {
							...
							ObjectManager.CreateObject(o, new World.Vector3D(0.0, 0.0, 0.0), new World.Transformation(0.0, 0.0, 0.0), new World.Transformation(0.0, 0.0, 0.0), true, 0.0, 0.0, 0.0);
						} catch (Exception ex) {
								...
								case Sdl.SDLK_F7: {
									...
									if (Dialog.ShowDialog() == DialogResult.OK) {
										...
										for (int i = 0; i < f.Length; i++) {
											...
											try {
												#endif
												ObjectManager.UnifiedObject o = ObjectManager.LoadObject(f[i], System.Text.Encoding.UTF8, ObjectManager.ObjectLoadMode.Normal, false, false, false);
												ObjectManager.CreateObject(o, new World.Vector3D(0.0, 0.0, 0.0), new World.Transformation(0.0, 0.0, 0.0), new World.Transformation(0.0, 0.0, 0.0), true, 0.0, 0.0, 0.0);
												#if !DEBUG
											} catch (Exception ex) {
