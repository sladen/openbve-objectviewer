		...
		private static void RenderOverlays() {
			...
			if (OptionInterface) {
				if (ObjectManager.ObjectsUsed == 0 & ObjectManager.AnimatedWorldObjectsUsed == 0) {
					...
					Keys = new string[][] { new string[] { "F" }, new string[] { "N" }, new string[] { "L" }, new string[] { "C" }, new string[] { "I" } };
					...
					RenderString((double)ScreenWidth - 28.0, 64.0, Fonts.FontType.Small, "Coordinate system: " + (Renderer.OptionCoordinateSystem ? "on" : "off"), 1, 1.0f, 1.0f, 1.0f, true);
					RenderString((double)ScreenWidth - 28.0, 84.0, Fonts.FontType.Small, "Hide interface", 1, 1.0f, 1.0f, 1.0f, true);
